...
type TText = string | { caption?: string; description?: string } | undefined

function fakeGallery(texts: TText[]): IGallery {
  // exactOptionalPropertyTypes: an absent caption means an absent key.
  const slides: ISlideData[] = texts.map((text, i) => {
    const slide: ISlideData = { key: `k${i}`, type: 'image', src: '' }
    const fields = typeof text === 'string' ? { caption: text } : (text ?? {})
    if (fields.caption !== undefined) {
      slide.caption = fields.caption
    }
    if (fields.description !== undefined) {
      slide.description = fields.description
    }
    return slide
  })
...
describe('registerCaption', () => {
  ...
  it('gives a slide two lines when it has both, and one when it has either', () => {
    // Native shows title and description as separate lines; a description
    // alone still earns the slide an item, on its own line below an empty
    // title — never promoted into the title's place.
    const pswp = fakePswp()
    registerCaption(
    ...
    const lines = (item: HTMLElement) =>
      [...item.children].map((c) => [c.className, c.textContent] as const)

    const [both, descriptionOnly, titleOnly] = items(pswp.uiElementAt(0)) as [
      HTMLElement,
      HTMLElement,
      HTMLElement
    ]
    expect(lines(both)).toEqual([
      ['arts-lightbox-caption__title', 'Title'],
      ['arts-lightbox-caption__description', 'Description']
    ])
    expect(lines(descriptionOnly)).toEqual([
      ['arts-lightbox-caption__description', 'Only a description']
    ])
    expect(lines(titleOnly)).toEqual([['arts-lightbox-caption__title', 'Only a title']])
  })
