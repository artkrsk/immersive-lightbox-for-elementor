...
describe('playing signals', () => {
  it('recognizes YouTube playback via infoDelivery playerState 1', () => {
    expect(
      youtubeProtocol.isPlayingEvent({ event: 'infoDelivery', info: { playerState: 1 } })
    ).toBe(true)
    expect(
      youtubeProtocol.isPlayingEvent({ event: 'infoDelivery', info: { playerState: 2 } })
    ).toBe(false)
    expect(youtubeProtocol.isPlayingEvent({ event: 'onReady' })).toBe(false)
    expect(youtubeProtocol.isPlayingEvent(null)).toBe(false)
  })

  it('recognizes Vimeo playback via its play events', () => {
    expect(vimeoProtocol.isPlayingEvent({ event: 'play' })).toBe(true)
    expect(vimeoProtocol.isPlayingEvent({ event: 'playProgress' })).toBe(true)
    expect(vimeoProtocol.isPlayingEvent({ event: 'ready' })).toBe(false)
    expect(vimeoProtocol.isPlayingEvent('garbage')).toBe(false)
  })
})
