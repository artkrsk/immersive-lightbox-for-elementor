...
export function fakePswp() {
  ...
  const pswp = {
    ...
    addFilter(name: string, fn: (...args: never[]) => unknown) {
      const list = filters.get(name) ?? []
      list.push(fn)
      filters.set(name, list)
    },
    /** Runs every registered filter for `name`, threading the value through each in turn. */
    applyFilters(name: string, initial: unknown, ...args: unknown[]): unknown {
      let value = initial
      for (const fn of filters.get(name) ?? []) {
        value = fn(value as never, ...(args as never[]))
      }
      return value
    },
    ...
    canLoop: () => false,
    /**
     * Slide count and the loop option, as the chrome reads them. Set both to
     * match the gallery a test hands `registerUi` — nothing keeps them in
     * sync automatically.
     */
    numItems: 2,
    options: { loop: false } as { loop?: boolean } & Record<string, unknown>,
    getNumItems(): number {
    ...
    /** The top bar, when a test needs the class the chrome stamps on it. */
