...
import { clamp, equalizePoints, roundPoint } from '../util/util'
...
class DragHandler {
  ...
  private _finishPanGestureForAxis(axis: 'x' | 'y'): void {
    const { velocity } = this.gestures
    const { currSlide } = this.pswp
   ...
   */
  private _panOrMoveMainScroll(axis: 'x' | 'y'): boolean {
    ...
    // @arts fork: a scroller with one item has nowhere to go, so dragging it
    // only rubber-banded back — motion that answers a gesture by undoing it.
    // `canBeSwiped` is upstream's own test for this, defined and never
    // called; both main-scroll moves below now ask it.
    const canSwipe = mainScroll.canBeSwiped()
    ...
    if (this.pswp.options.artsMouseDragNavigates && this.gestures.isMousePointer) {
      if (axis === 'x' && !isMultitouch && canSwipe) {
   ...
   */
  private _getVerticalDragRatio(panY: number): number {
