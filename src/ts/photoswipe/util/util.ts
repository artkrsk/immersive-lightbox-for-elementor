...
/* decode() is missing from ancient engines — modeled optional here (lib.dom
   marks it required), which is the exact reason for the runtime guard. A
   truthiness guard rather than `in`: it narrows the optional away under every
   consumer tsconfig, not only with `exactOptionalPropertyTypes`. */
