...
export interface PhotoSwipeEventsMap {
  ...
  change: undefined
  /**
   * @arts fork — dispatched by MainScroll.moveIndexBy once the destination
   * is decided (press, drag release), before the strip lands; `change` only
   * follows at spring completion. `direction` is the loop-corrected travel
   * sign: a wrap reports the short way round.
   */
  potentialIndexChange: { direction: 1 | -1 }
  afterInit: undefined
