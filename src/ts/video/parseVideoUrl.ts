...
//
// "videoseries" is the playlist-embed pseudo-id and is exactly 11 characters,
// so it slips through the id shape — we don't carry playlists, so reject it
// rather than build an embed that errors.
const YOUTUBE =
  /(?:youtube(?:-nocookie)?\.com\/(?:watch\?[^#]*?v=|embed\/|shorts\/|live\/|v\/)|youtu\.be\/)(?!videoseries)([\w-]{11})/i
// The prefixes users actually paste: player, dashboard (manage/videos),
// showcases and legacy albums ({name}/{id}/video), on-demand, channels,
// groups. The trailing group is the private hash, told apart from a trailing
// path word (/likes, /settings) by length plus at least one digit — real
// hashes are 10 hex characters.
const VIMEO =
  /vimeo\.com\/(?:(?:channels|ondemand)\/[\w-]+\/|groups\/[\w-]+\/videos\/|manage\/videos\/|(?:[\w-]+\/)?(?:\d+\/)?video\/)?(\d+)(?:\/(?=[a-z\d]*\d)([a-z\d]{8,}))?/i
