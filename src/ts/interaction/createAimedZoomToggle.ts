 ...
 * with the pan continuously aimed at the live mouse each frame — landing
 * exactly where the pointer is, so the following mousemove is seamless. (At
 * fit the pan range is degenerate, so the same mapping centers
 * automatically.) The zoomed level is wherever a click lands: fill under the
 * fill model (the slide opened there), the click level under the fit model
 * — `max(initial, secondary)`, the same reading canSlideZoom takes.
 */
export function createAimedZoomToggle(
  ...
  const toggle = (): void => {
    ...
    // A guessed box's range is a placeholder about to be replaced, and a
    // collapsed one has nowhere to go — neither is worth animating toward.
    ...
    const dest = from > fit + FIT_EPSILON ? fit : Math.max(initial, secondary)
    onStart()
