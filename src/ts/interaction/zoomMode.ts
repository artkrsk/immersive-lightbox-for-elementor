...
export function attachZoomMode(pswp: PhotoSwipe, opts: IOptions): void {
  ...
  // classic model (zoom mode 'fit' → click level ~3x fit) the threshold
  // would fire on any deep zoom and rewrite the live options, collapsing
  // the range to the fill ceiling; with zoom off there is no range at all.
  if (opts.zoom.mode !== 'fill') {
    return
  }
  let mode: 'fit' | 'fill' = 'fill'

  const syncSlide = (slide: NonNullable<PhotoSwipe['currSlide']>): void => {
    // The mode is what the USER prefers; a slide that cannot honor it shows
    // at fit instead. Nothing resets the preference, which is what lets the
    // next slide with a real range pick it straight back up.
    const target = canSlideZoom(slide) ? slide.zoomLevels[mode] : slide.zoomLevels.fit
  ...
  pswp.on('zoomPanUpdate', (e) => {
    const slide = e.slide
    // Only a slide that can really zoom may speak for the user's preference:
    // an interim box's range is invented, and reading a level off it would
    // record a choice nobody made.
    if (!canSlideZoom(slide)) {
      return
    }
  ...
  // The naturals land long after a guessed slide painted (loadComplete, not
  // change), and they can turn a fictional range into a real one or into
  // none at all. Re-apply there, or the slide keeps the interim decision.
  pswp.on('zoomLevelsUpdate', () => {
    const slide = pswp.currSlide
    if (slide) {
      syncSlide(slide)
    }
