 ...
 * trackpad pinch — remains PhotoSwipe's zoom (inert under zoom mode 'off',
 * where the zoom policy makes every slide non-zoomable).
 *
 * Tied to explore because without it the wheel pans a zoomed slide, which
 * a swipe would fight. With zoom off nothing is ever pannable, so the wheel
 * is free to navigate whatever explore says.
 */
export function attachWheelNav(pswp: PhotoSwipe, opts: IOptions): void {
  const navigates = opts.zoom.mode === 'off' || (opts.explore.enabled && !opts.zoom.wheelToZoom)
  if (!navigates) {
    return
  }
